...
runSimpler state (DAssign var exp)         = extend state var $ eval state exp
runSimpler state (DIf exp stat_tr stat_fl) = if eval state exp == 1
                                             then runSimpler state stat_tr
                                             else runSimpler state stat_fl
runSimpler state DSkip                     = state
runSimpler state (DWhile exp stat)         = if eval state exp == 1
                                             then runSimpler stat_new (DWhile exp stat)
                                             else state
                                             where stat_new = runSimpler state stat
runSimpler state (DSequence stat1 stat2)   = runSimpler stat_new stat2
                                             where stat_new = runSimpler state stat1
...
run state stat = runSimpler state $ desugar stat
